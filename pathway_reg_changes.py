...
diff_stats = diff_stats.dropna(subset=["pathways"]).reset_index(drop=True)
diff_stats


#finding mean average expression of each pathway
#use the diff_copy column - just easier
median_expression_change = diff_stats.groupby("pathways").diff_copy.median().reset_index()
median_expression_change
median_expression_change.to_csv('median.csv')

colors = ["red" if x > 0 else "blue" for x in median_expression_change["diff_copy"]]

plt.figure(figsize=(10, 6))
sns.barplot(data=median_expression_change, x="pathways", y="diff_copy", palette=colors)
plt.xlabel("Pathway")
plt.ylabel("Median Expression Change")
plt.title("Median Expression Change by Pathway")
...
#looking at mean expression change for each pathway
mean_expression_change = diff_stats.groupby('pathways').diff_copy.mean().reset_index()
mean_expression_change.to_csv('mean.csv')

colors1 = ["red" if x > 0 else "blue" for x in mean_expression_change["diff_copy"]]

plt.figure(figsize=(10, 6))
sns.barplot(data=mean_expression_change, x="pathways", y="diff_copy", palette=colors1)
plt.xlabel("Pathway")
plt.ylabel("Mean Expression Change")
plt.title("Mean Expression Change by Pathway")
plt.xticks(rotation=45, ha="right")  # Rotate x-axis labels for better readability
plt.tight_layout()
plt.show()


#min protein changes (most downregulated)
min_diff_protein = diff_stats.loc[diff_stats.groupby('pathways')['diff_copy'].idxmin(), ['pathways', 'protein', 'diff_copy']]
min_diff_protein

plt.figure(figsize=(10, 6))
sns.barplot(data=min_diff_protein, x="pathways", y="diff_copy")
...
#violin plot of each pathway and the differences as the points
plt.figure(figsize=(12,6))
sns.violinplot(data=diff_stats, x='pathways', y='diff_copy', palette='muted')

plt.title("Distribution of Protein Abundance Changes by Pathway", fontsize=14)
...
plt.figure(figsize=(12,6))
sns.violinplot(data=diff_stats_sf, x='pathways', y='diff_copy', palette='muted')
plt.title("Distribution of Protein Abundance Changes by Pathway - signficant proteins", fontsize=14)
plt.xlabel("Pathway", fontsize=12)
plt.ylabel("Abudnance Change (LogFC)", fontsize=12)
plt.xticks(rotation=45, ha="right")
...
#median change but only significant proteins
sf_median_exp = diff_stats_sf.groupby('pathways').diff_copy.median().reset_index()
sf_median_exp.to_csv('sf_median.csv')

colors2 = ["indianred" if x > 0 else "steelblue" for x in sf_median_exp["diff_copy"]]

plt.figure(figsize=(10, 6))
sns.barplot(data=sf_median_exp, x="pathways", y="diff_copy", palette=colors2)
plt.xlabel("Pathway")
plt.ylabel("Median Expression Change")
plt.title("Median Expression Change by Pathway - significant proteins")
plt.xticks(rotation=45, ha="right")  # Rotate x-axis labels for better readability
plt.tight_layout()
plt.show()


#mean change but only significant proteins
sf_mean_exp = diff_stats_sf.groupby('pathways').diff_copy.mean().reset_index()
sf_mean_exp.to_csv('sf_mean')

colors3 = ["indianred" if x > 0 else "steelblue" for x in sf_mean_exp["diff_copy"]]

plt.figure(figsize=(10, 6))
sns.barplot(data=sf_mean_exp, x="pathways", y="diff_copy", palette=colors3)
plt.xlabel("Pathway")
plt.ylabel("Mean Expression Change")
...
#min change in protein abundance - significant proteins
min_diff_protein_sf = diff_stats_sf.loc[diff_stats.groupby('pathways')['diff_copy'].idxmin(), ['pathways', 'protein', 'diff_copy']]
min_diff_protein_sf

plt.figure(figsize=(10, 6))
sns.barplot(data=min_diff_protein_sf, x="pathways", y="diff_copy")
plt.xlabel("Pathway")
plt.ylabel("Expression Change LogFC")
plt.title("Min Expression Change by Pathway - significant proteins")
plt.xticks(rotation=45, ha="right")  # Rotate x-axis labels for better readability
plt.show()
...
#boxplot of pathway changes but from experimental data
PBRM1_data3 = pd.read_excel('~/icr/simko/data/pbrm1_experiment_data/all_pbrm1_results.xlsx')
PBRM1_data3.columns = PBRM1_data3.columns.str.replace(' ', '_').str.replace('-', '').str.replace('/', '_')
PBRM1_data3

#create a data frame with only gene names, logfc and log ttest pvalue
PBRMdata_cond = PBRM1_data3[['Gene_Names_(primary)', 'mean_log2_KO_WT', 'Log_Ttest_pvalue_']].copy()

#now i need to add patwhays to this data frame
PBRMdata_cond['pathways'] = PBRMdata_cond['Gene_Names_(primary)'].apply(lambda protein: [
    pathway
    for pathway, proteins in {
        "allograft_rejection": allograft_rejection,
        "apoptosis": apoptosis,
...
#duplicating/tripling etc protein rows that appear in multiple pathways
PBRMdata_cond = PBRMdata_cond.explode("pathways").reset_index(drop=True)
#getting rid of nan rows
PBRMdata_cond = PBRMdata_cond.dropna(subset=["pathways"]).reset_index(drop=True)
PBRMdata_cond
