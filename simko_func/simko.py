...
def get_classes_by_mean_abundance(abundance, ko_proteins, n):
    ...
    ### Select protein columns with proteins to be KO'd 
    ko_proteins_abundance = abundance.loc[abundance.index.isin(ko_proteins)]
    ko_proteins_abundance = ko_proteins_abundance.T
    ### Take the mean abundance of KO proteins for every cell line and sort data by mean(This will commonly be a single KO)
    ko_proteins_abundance['mean'] = ko_proteins_abundance.mean(axis=1, numeric_only=True, skipna=True)
    ko_proteins_abundance = ko_proteins_abundance.dropna().sort_values("mean", ascending = False)
    ### Extract median group cell lines (a subset of the last n cell lines of the top half of the sorted data) and label as median
    median = ko_proteins_abundance.head(round((ko_proteins_abundance.shape[0]/2))).tail(n).copy()  # Should take middle n rows
    # median = ko_proteins_abundance.tail(round((ko_proteins_abundance.shape[0]/2) + n/2)).head(n).copy()  # Should take middle n rows
    median.loc[:,'class'] = 'median'
    ### Extract low group cell lines (bottom n in sorted data) and label as low
    low = ko_proteins_abundance.tail(n).copy()
    low.loc[:,'class'] = 'low'
    ### Merge mediam and low rows and fix index names
    protein_classes = pd.concat([median, low])
    protein_classes.columns.name = None
    protein_classes.index.name = 'cl'
    return pd.DataFrame(protein_classes)

def get_ko_differentials(abundance, class_df):
    """Returns PD dataframe of abundance differences between classes given by class_df"""
    median_class = list(class_df.loc[class_df['class']=='median'].index)
    low_class = list(class_df.loc[class_df['class']=='low'].index)
    diff_df = pd.DataFrame()
    diff_df['median'] = abundance.filter(median_class).mean(axis=1, numeric_only=True, skipna=True)
    diff_df['median_std'] = abundance.filter(median_class).std(axis=1, numeric_only=True, skipna=True)
    diff_df['low'] = abundance.filter(low_class).mean(axis=1, numeric_only=True, skipna=True)
    diff_df['low_std'] = abundance.filter(low_class).std(axis=1, numeric_only=True, skipna=True)
    diff_df['diff'] = (diff_df['low'] - diff_df['median'])
    diff_df = diff_df.reset_index()
    return diff_df

def get_control_differentials(abundance, class_df=None, trials=100, k=30):
    """Returns PD dataframe with control abundance differences based on random CL selections (With [trials] * trials of size k)"""
    ko_effect_control = pd.DataFrame()
    cls = abundance.columns
    print(cls)
    # mid_cl = list(class_df.loc[class_df['class']=='median'].index)
    for protein_ko in ['control_' + str(i) for i in range(trials)]:
        mid_cl = random.choices(cls, k=k)
        sample_cl = random.choices(list(set(cls) - set(mid_cl)), k=k) #list(classes.sample(30)['cl'])
        abundance_stats = pd.DataFrame()
        mid_abundance = abundance[mid_cl]
        abundance_stats['mid_mean'] = mid_abundance.mean(axis=1)
        sample_abundance = abundance[sample_cl]
        abundance_stats['sample_mean'] = sample_abundance.mean(axis=1)
        abundance_stats['diff'] = abundance_stats['sample_mean'] - abundance_stats['mid_mean']
        abundance_stats['ko'] = protein_ko
        abundance_stats = abundance_stats[['ko', 'diff']]
        abundance_stats = abundance_stats.reset_index()
        abundance_stats = abundance_stats.dropna()
        ko_effect_control = pd.concat([ko_effect_control, abundance_stats])
    return(ko_effect_control)

def get_significance(diff_df, control, n=30):
    def ttest_from_stats(row, n = 30):
        ko_std = 1.96
        pooled_sd = np.sqrt((((n-1)*(ko_std**2)) + ((n-1)*(row['control_std']**2))) / (n + n - 2))
        t_stat = (row['diff'] - row['control_mean']) / (pooled_sd * np.sqrt(1/n + 1/n))
        p_value = 2*(1 - t.cdf(abs(t_stat), (n + n - 2)))
        return p_value
    
    def ttest_from_sample_stats(row, n=30):
        sample = row['diff']
        control_mean = row['control_mean']
        control_std = row['control_std']
        t_stat = (sample - control_mean) / (control_std / np.sqrt(n))
        p_value = t.sf(abs(t_stat), df=n-1)*2
        return p_value  #{'t_stat':t_stat,  'p_value':p_value}

    control = control[['protein', 'diff']].groupby('protein').agg(['mean', 'std']).reset_index()
    control.columns = ['protein', 'control_mean', 'control_std']
    diff_df = diff_df.merge(control, how='left', left_on='protein', right_on='protein')
    diff_df['p'] = diff_df.apply(ttest_from_sample_stats, n=n, axis=1)
    #bonferroni test on pvalues
    m = len(diff_df) #total number of "tests"
    diff_df['adjusted_p'] = np.minimum(diff_df['p']*m, 1.0)
...
def get_all_simko(abundance, control_diffs, proteins, n=30):
    protein_count = len(proteins)
    i = 1
    diffs_list = []
    for ko_prot in proteins:
        print("%s / %s - %s " % (i, protein_count, ko_prot),i,sep='',end="\r",flush=True)
        try:
            class_df = get_classes_by_mean_abundance(ko_proteins=[ko_prot], abundance=abundance, n=30)
            ko_diffs = get_ko_differentials(abundance=abundance, class_df=class_df)
            ko_diffs = get_significance(ko_diffs , control_diffs, n=30)
            ko_diffs['ko'] = ko_prot
...
def get_differentials_boxplot(class_df, data_df, protein_list):
    # median_class = list(class_df.loc[class_df['class']=='median'].index)
    # low_class = list(class_df.loc[class_df['class']=='low'].index)
...
def filter_cls(abundance, model_annotation, cancer_types, data_source = 'passport'):
    cl_list = model_annotation.loc[model_annotation['cancer_type'].isin(cancer_types)] ### Single SimKO
    if data_source == 'passport':
        cl_list = list(cl_list['model_name'])
    else:
        cl_list = list(cl_list['CCLE_ID'])
    abundance_filt = abundance.loc[:,abundance.columns.isin(cl_list)]
    return abundance_filt


#function for getting pathways assigned to the proteins
def get_pathway_assigned(diff_stats, operation='mean'):
    ...
    diff_stats1 = diff_stats.explode("pathways").reset_index(drop=True)
    diff_stats_assigned = diff_stats1.dropna(subset=["pathways"]).reset_index(drop=True) 
...
def get_pathway_stats(diff_stats_assigned, operation='mean'):
    if operation == 'mean':
        ...
        get_pathway_stats = diff_stats_assigned.groupby('pathways').diff_copy.median().reset_index()
    elif operation == 'min':
        get_pathway_stats = diff_stats_assigned.groupby('pathways').diff_copy.min().reset_index()    
    return get_pathway_stats


#function to plot boxplot for pathway regulation changes
#the data will need to be 
def boxplots_for_pathways(data, x, y, 
                          pathways_to_color=['myc_targets', 'g2m_checkpoint', 'dna_repair', 'e2f_targets'],
                          title=None, xlabel=None, ylabel=None,
                          save_path=None,
                          ylim=None,
                          figsize=(10,8)):
    plt.figure(figsize=figsize)
    unique_pathways = data[x].unique()
    palette = {pathway: 'lightblue' if pathway in pathways_to_color else 'red' for pathway in unique_pathways}
    pathway_order = sorted(data[x].unique())
    sns.boxplot(data=data, x=x, y=y, palette=palette, order=pathway_order)
